...
/**
 * @author 이성수
 * @brief 기존의 XRInteractorLineVisual을 그대로 복제하여, Invalid 시 Line Renderer의 Width값을 함께 변경하는 기능을 추가한 클래스입니다.
 * @details 기존의 XRInteractorLineVisual 클래스가 오버라이드 불가능, 패키지라서 수정도 불가능하기에 복제라는 방법을 선택...
 * @since 2023-03-26
 */
[DisallowMultipleComponent]
[RequireComponent(typeof(LineRenderer))]
[DefaultExecutionOrder(XRInteractionUpdateOrder.k_LineVisual)]
public class CustomInteractorLineVisual : MonoBehaviour, IXRCustomReticleProvider
{
	...
	/// <summary>
	/// 유효하지 않은 경우 라인 Width를 바꾸는 기능
	/// </summary>
	/// <author>이성수</author>
	/// <since>2023-03-26</since>
	[SerializeField, Range(k_MinLineWidth, k_MaxLineWidth)]
	float m_InvalidLineWidth = 0.005f;
	...
	void UpdateLineVisual()
	{
		...
		else
		{
			...
			// 2023-03-26 추가
			m_LineRenderer.widthMultiplier = Mathf.Clamp(m_InvalidLineWidth, k_MinLineWidth, k_MaxLineWidth);
